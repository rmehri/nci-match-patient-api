namespace :setup do
  ...
  task :create_table, [:name] => :before do | t, args |
    args.to_a.each do | arg |
      create_table("NciMatchPatientModels::#{arg.classify}".constantize)
    end
  end

  task :delete_table, [:name] => :before do | t, args |
    args.to_a.each do | arg |
      delete_table("NciMatchPatientModels::#{arg.classify}".constantize)
    end
  end

  task :clear_table, [:name] => :before do | t, args |
    args.to_a.each do | arg |
      clear_table("NciMatchPatientModels::#{arg.classify}".constantize)
    end
  end
