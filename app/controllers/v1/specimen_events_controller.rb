  ...
  class SpecimenEventsController < BaseController
    ...
    def embed_resources(resource ={})
      ...
      resource[:specimen_shipments].collect do | shipment |
        shipment[:analyses] = NciMatchPatientModels::Assignment.scan(build_query({:molecular_id => shipment[:molecular_id], :projection => [:status, :status_date, :comment_user,:comment]})).collect{|record| record.to_h.compact}
