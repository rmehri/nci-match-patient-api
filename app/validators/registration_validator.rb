...
module RegistrationValidator
  ...
  included do
    ...
    # validates :status_date, presence: true, date: { on_or_before: lambda {DateTime.current.utc}, message: "registration_date can not be later than current date"}
    validates :status_date, presence: true, inclusion: { in: ((Time.now.utc - 5.years)..Time.now.utc), message: "registration_date can not be later than current date" }
